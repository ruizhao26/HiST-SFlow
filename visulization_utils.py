...
def flow_to_img(flow_uv, clip_flow=None, convert_to_bgr=True):
    """
...
def vis_flow_batch(forw_flows, save_path, suffix='forw_flow', max_batch=4):
    # forward flow
    for ii in range(forw_flows.shape[0]):
        flow = forw_flows[ii, :]
        flow = flow.permute([1, 2, 0]).detach().cpu().numpy()
        if ii == 0:
            flow_viz = flow_to_img(flow, convert_to_bgr=True)
        else:
            flow_viz = np.concatenate([flow_viz, flow_to_img(flow, convert_to_bgr=True)], axis=1)        
        if ii-1 >= max_batch:
            break
